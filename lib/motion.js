(function(ns) {
  var now            = (new Date()).getTime(), // current time
      ...
      motion         = function(mode, config) {
        ...
        switch (mode) {
          case motion.SERVER:
            var delta = false, lastTick = 0;

            // TODO:
            // + setup a delta queue
            context.lastSnapshotTime = now;
            tickOperations.push(function(time) {
              if (time - context.lastSnapshotTime >= context.config.syncRate) {
                chain.emit('sync', delta);
                context.lastSnapshotTime = time;
              }
            });
        ...
        chain.ticker = {
          ...
          start : function() {
            ...
            tickTimer = setInterval(function() {
              ...
              for (i; i<l; i++) {
                if (typeof tickOperations[i] === 'function') { // TODO: is this needed?
          ...
          if (!context.lastControllerSync) {
            ...
            tickOperations.push(function(time) {
              ...
              if (time - context.lastControllerSync >= context.config.syncRate)
              {
                ...
                chain.emit('sync', netMsg('controllers', events));
                context.lastControllerSync = now;
              }
            });
          }
